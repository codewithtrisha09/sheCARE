# SheCARE

Empowering Teen Girls Through Health Education and Wellness.

## About the Project

SheCARE is a health-tech web application designed to promote menstrual health awareness and provide accessible educational resources for adolescent girls. The platform aims to create a safe and informative space where users can learn about menstrual wellness through engaging content and a user-friendly interface.

## Current Features

* Modern and responsive user interface
* Health and menstrual wellness information pages
* Educational blog/content modules
* Intuitive navigation and user experience
* Custom branding and design

## Tech Stack

### Frontend

* React.js
* Vite
* HTML5
* CSS3
* JavaScript (ES6+)

## Project Status

🚧 Work in Progress

The current version focuses on frontend development, UI/UX design, and educational content modules. Additional features and backend functionality are planned for future releases.

### Completed

* React + Vite setup
* Landing page design
* Responsive navigation bar
* Menstrual health information section
* Educational blog/content module
* Custom branding and logo
* Responsive user interface

### Planned Features

* Period tracking system
* User authentication
* Personalized health dashboard
* Health reminders and notifications
* AI-powered health assistant
* Medical report simplification tools

## Installation

```bash
git clone https://github.com/codewithtrisha09/sheCARE.git
cd sheCARE
npm install
npm run dev
```

## Author

**Trisha Shetty**

B.Tech CSE (AI & ML)

Building technology-driven solutions to improve health awareness and accessibility for young women.
